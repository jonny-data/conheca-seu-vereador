...
class HTTPConnection {
	...
	public function __construct() {
		if (self::$userAgent == null) {
			...
			if (function_exists('posix_uname')) {
				...
				self::$userAgent = sprintf('Mozilla/4.0 (compatible; %s; PHP/%s; %s; %s)',
											PHP_SAPI,
											PHP_VERSION,
											PHP_OS,
											$locale);
	...
	public function setConnectionTimeout($connectionTimeout) {
		if (is_integer($connectionTimeout)) {
			...
			throw new InvalidArgumentException('Connection timeout must be specified in seconds.');
		}
	}
	...
	public function setTimeout($timeout) {
		if (is_integer($timeout)) {
			...
			throw new InvalidArgumentException('Timeout must be specified in seconds.');
		}
	}
}